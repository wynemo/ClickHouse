...
def _run_test(job_name: str, run_command: str) -> int:
    ...
    if not run_command:
        ...
        run_command = "./tests/ci/" + CI_CONFIG.get_job_config(job_name).run_command
...
def main() -> int:
    ...
    ### RUN action: start
        ...
        # """
        # returns:  10 if rerun for success job
        #          -10 if rerun for failed job
        #           20 if no run_command configured - pass job execution to yaml
        #           X   if run_command is configured return it's exit code
        # """
        ...
        #     print("Check is already finished according to github status, exiting")
